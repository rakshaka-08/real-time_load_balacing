...
class LPTTests(unittest.TestCase):
    ...
    def test_longest_tasks_first(self):
        result = LPTAlgorithm(self.problem).optimize()

        self.assertEqual(
            result.schedule.task_order,
            ("a", "b", "c"),
        )
        self.assertEqual(result.algorithm, "LPT")
    ...
    def test_known_heterogeneous_schedule(self):
        ...
        self.assertAlmostEqual(
            result.schedule.total_response_time, 6.5
        )
        self.assertAlmostEqual(result.schedule.makespan, 2.5)
    ...
    def test_future_task_does_not_delay_earlier_arrival(self):
        problem = SchedulingProblem(
            [
                TaskSpec("later-long", 1000, arrival_time=10),
                TaskSpec("earlier-short", 250, arrival_time=0),
            ],
            [VMSpec("vm", 500)],
        )
        ...
        self.assertEqual(first.task_id, "earlier-short")
        self.assertEqual(first.start_time, 0)
        self.assertEqual(second.task_id, "later-long")
    ...
    def test_seed_does_not_change_schedule(self):
        first = LPTAlgorithm(self.problem, seed=1).optimize()
        second = LPTAlgorithm(self.problem, seed=999).optimize()
